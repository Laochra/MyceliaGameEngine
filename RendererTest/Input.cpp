...
Input::~Input()
{
    ...
    delete keyCodeToName;
    delete keyNameToCode;
}
...
{
    const char* currentKey = input->keyCodeToName->find(keybind->GetBinds()[0])->second;
    ...
    if (ImGui::BeginCombo(label, currentKey))
    {
        for (int i = 0; i < keyAmount; i++)
        {
            ...
            if (ImGui::Selectable(keys[i], isCurrent))
            {
                currentKey = keys[i];
                keybind->ClearBinds();
                keybind->Bind(input->keyNameToCode->find(keys[i])->second);
            }
...
{
    const char* currentNegativeKey = input->keyCodeToName->find(keyAxis->GetNegativeBinds()[0])->second;
    const char* currentPositiveKey = input->keyCodeToName->find(keyAxis->GetPositiveBinds()[0])->second;
    ...
    if (ImGui::BeginCombo(negativeLabel, currentNegativeKey))
    {
        for (int i = 0; i < keyAmount; i++)
        {
            ...
            if (ImGui::Selectable(keys[i], isCurrent))
            {
                ...
                keyAxis->BindNegative(input->keyNameToCode->find(keys[i])->second);
            }
    ...
    if (ImGui::BeginCombo(positiveLabel, currentPositiveKey))
    {
        for (int i = 0; i < keyAmount; i++)
        {
            ...
            if (ImGui::Selectable(keys[i], isCurrent))
            {
                ...
                keyAxis->BindPositive(input->keyNameToCode->find(keys[i])->second);
            }
